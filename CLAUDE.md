...
This is a macOS menu bar app that bridges Claude Code's `/ide` WebSocket MCP protocol to Xcode's `xcrun mcpbridge` STDIO MCP server.

```
Claude Code CLI ──WebSocket (MCP)──> XcodeIDEAdapter
